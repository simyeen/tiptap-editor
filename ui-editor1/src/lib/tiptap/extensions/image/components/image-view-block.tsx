...
import {blobUrlToBase64, randomId} from '../../../util';
...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  const containerMaxWidth = containerRef.current
    ? parseFloat(getComputedStyle(containerRef.current).getPropertyValue('width'))
    : Infinity;
  ...
  const targetMaxWidth = containerMaxWidth > 2 ? containerMaxWidth - 2 : 0;
    ...
    >
      <div
        className={cn('group/node-image relative rounded-md object-contain', {
          'mx-auto': imageState.align === 'center',
          'mr-auto': imageState.align === 'left',
          'ml-auto': imageState.align === 'right',
        })}
        style={{
        ...
        >
          ...
          {!isResizing && !imageState.error && !imageState.isServerUploading && (
            <ImageActions
              ...
              onAlignCenter={() => setImageState({...imageState, align: 'center'})}
              onAlignLeft={() => setImageState({...imageState, align: 'left'})}
              onAlignRight={() => setImageState({...imageState, align: 'right'})}
              editable={editable}
            />
          )}
        </div>
